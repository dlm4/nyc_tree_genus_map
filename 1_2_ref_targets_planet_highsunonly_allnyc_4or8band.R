...
# Prep input stuff
num_bands <- 8 # 4 or 8
path_planet_mosaics <- paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_", num_bands, "b_highsunonly")

# to write out
output_prefix <- paste0("all_nyc_", num_bands,"b_")
output_target_refl_dir <- paste0("/Volumes/NYC_geo/Planet/tests/nyc_daily_stack_", num_bands, "b_highsunonly_cal") # no trailing "/"
...
# 20220710 has complete coverage over all NYC and is very clear and close to the summer solstice

# To setup, do regressions for each image, band
ref_date <- all_target_df[which(all_target_df$date == ymd("20220710")),]
