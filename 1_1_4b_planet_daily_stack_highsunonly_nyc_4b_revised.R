...
# Doing all 4b files that had max 10% cloud

# Keep only files with:
#   > 20 solar elevation angle
#   Only best image per day for a given location (based on highest solar angle because clear_percent is sometimes missing in the json)
...
# Do the cloud masking first and then this will fill in the holes with anything that's underneath if using na.rm()
...
# date range
date_range <- seq(ymd("20160101"), ymd("20250228"), "days")
